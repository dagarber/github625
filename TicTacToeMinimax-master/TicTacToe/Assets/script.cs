...
public class script : MonoBehaviour {
	...
	public int minimax(int depth, int turn) {
		// RISK OF INFINITY LOOP WHEN YOU USE RECURSION (THE FUNCTION CALLS ITSELF)
		if (hasXWon()) return +1; //+1 for a player win // MUST HAVE AN EXIT CONDITION BEFORE CALLING THE FUNCNTION YOU ARE IN
		if (hasOWon()) return -1; //-1 for a computer win // (RECURSIVELY)
		
		List<MNode> pointsAvailable = getMoves(); // get the empty spaces that are left
		if (pointsAvailable.Capacity == 0) return 0;

		// FIRST, SPECIFY WHEN YOU WILL FINISH (ABOVE), THEN DO THE WORK OF THE JOB
		...
		for (int i = 0; i < pointsAvailable.Count; i++) {
			...
			if (turn == 1) { 
				...
				int currentScore = minimax(depth + 1, 2); // RECURSIVE CALL
		...
		return turn == 1 ? returnMax(scores) : returnMin(scores); // one line if statement
		// if turn is 1, return returnMax(scores), else return returnMin(scores)
	}
}
